...
__global__ void cuda_ising2(int *G, int *G1, int n){
    ...
    if(i < n && j < n){
        ...
        for(int i1 = i;i1 < i + momblocksize;i1++){
            for(int j1 = j;j1 < j + momblocksize;j1++){
                ...
                int sum = G[iup*n + j1] + G[i1*n + jleft] + G[i1*n + j1] + 
                        G[idown*n +j1] + G[i1*n +jright];
                G1[i1*n + j1] = (sum > 0) - (sum < 0);    //implemets sign function
            }
        }
    }
}  
...
int main(int argc, char **argv){
    ...
    for(int itr = 0;itr < K; itr++){
        if(!(itr % 2)) cuda_ising2 <<<dimGrid, dimBlock>>> (d_G, d_G1, N);
        else cuda_ising2 <<<dimGrid, dimBlock>>> (d_G1, d_G, N);
    }
